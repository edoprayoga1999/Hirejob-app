...
function Login() {
  ...
  const loginSubmit = (e) => {
    ...
    if (!form.email || !form.password) {
      ...
      axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/login/recruiter`, form)
        .then((result) => {
  ...
  return (
    <div className="w-50 d-flex flex-column justify-content-center align-items-center" style={{ height: '100vh', overflow: 'auto' }}>
      ...
      >
        Silahkan login dengan akun perekrut anda.
      </p>
      ...
      <button type='button' className={style.loginButton} onClick={(e) => { loginSubmit(e); }}>Masuk sebagai perekrut</button>
      <div className="d-flex">
        <p style={{ fontSize: '16px', color: '#1F2A36' }}>Anda belum punya akun perekrut?&nbsp;</p>
        <Link href="/register/recruiter"><p className={style.cursorChange} style={{ fontSize: '16px', color: '#FBB017' }}>Daftar disini</p></Link>
      </div>
      <div className="d-flex">
        <p style={{ fontSize: '16px', color: '#1F2A36' }}>Masuk sebagai jobseeker?&nbsp;</p>
