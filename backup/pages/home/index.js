  ...
  const getListWorker = async () => {
    try {
      const response = await axios({
        method: 'GET',
        url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/allusers?name=${search}&field=${field}&type=${sortType}`,
        headers: {
          token
        }
      })
...
function Home(props) {
  ...
  const searchSubmit = (e) => {
    e.preventDefault()
    router.push(`/home?search=${search}&field=${field}`)
  }
  return (<>
    ...
    {props.workerList.error ? (<div>Error</div>) : listWorker.length === 0 ? (<div>Data not found</div>) : listWorker.map((item, index) => (<div key={index} className='d-flex flex-column' style={{ width: '80%', boxShadow: '0px 1px 20px rgba(197, 197, 197, 0.25)', borderRadius: '10px', padding: '40px 20px 40px 20px', marginBottom: '10px' }}>
        <div className='d-flex align-items-center w-100'>
          <div style={{ width: '10%' }}>
            <div style={{ width: '100px', height: '100px', borderRadius: '999px', position: 'relative', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundImage: `url('http://localhost:4000/${item.photo}')` }} />
          </div>
