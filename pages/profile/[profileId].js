...
function DetailProfile(props) {
  ...
  const sentHiringMessage = (e) => {
    ...
    axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/chat/add`, newData, {
      headers: {
        token: props.token
      }
    })
    ...
    <div className='d-flex w-100 position-absolute' style={{top: '170px'}}>
      <div style={{ width: '30%', display: 'flex' }}>
        <div style={{ width: '70%', backgroundColor: '#FFFFFF', marginLeft: 'auto', marginRight: '0px', padding: '30px', position: 'relative', borderRadius: '10px' }}>
          ...
          {props.level === '1' ? (<button type='button' onClick={(e) => { sentHiringMessage(e) }} style={{ width: '100%', padding: '15px', backgroundColor: '#5E50A1', color: '#FFFFFF', borderRadius: '5px', border: 'none', marginBottom: '40px' }}>
            {loading ? (<><FontAwesomeIcon icon={faSpinner} spin />&nbsp;Loading</>) : "Hire"}
          </button>) : ""}
